...
inline bool checkBounds(const pair<int, int> &p1, const int &lowerBound, const int &upperbound) {
    ...
    for (const auto &a : locations) {
        ...
        for (int i = 0; i < coords.size(); i++) {
            for (int j = i + 1; j < coords.size(); j++) {
                ...
                if (checkBounds(antinode1, 0, map.size())) {
                    antinodes.insert(antinode1);
                    cout << antinode1.first << "," << antinode1.second << "  ";
                }
                if (checkBounds(antinode2, 0, map.size())) {
